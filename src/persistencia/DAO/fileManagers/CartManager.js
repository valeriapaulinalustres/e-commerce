...
export default class CartManager {
  ...
  async addProductToCart(cid, pid) {
    try {
      ...
      const productInCart = cartFoundById.products.find(
        (el) => el.id === parseInt(pid)
      );
      const cartIndex = cartsFromFile.findIndex((el) => el.id === cid);
      const productIndex = cartsFromFile[cartIndex].products.findIndex(
        (el) => el.id === parseInt(pid)
      );
      if (productInCart) {
        ...
        cartsFromFile[cartIndex].products.push({
          id: parseInt(pid),
          quantity: 1,
        });
        await fs.promises.writeFile(this.path, JSON.stringify(cartsFromFile));
